...
def compute_quality_flags(
    ...
    constant_column_threshold: float = 0.99
) -> Dict[str, Any]:
    """
    ...
    for col in summary.columns:
        if col.unique == 1 and col.missing_share < constant_column_threshold:
    ...
    for col in summary.columns:
        # Проверяем категориальные признаки (строковые или категории)
        is_categorical = not col.is_numeric
        if is_categorical and col.unique > high_cardinality_threshold:
    ...
    flags["high_cardinality_count"] = len(high_cardinality_columns)
    ...
    if flags["has_constant_columns"]:
        score -= 0.1 * min(1.0, flags["constant_columns_count"] / summary.n_cols if summary.n_cols > 0 else 0)
    if flags["has_high_cardinality_categoricals"]:
        score -= 0.1 * min(1.0, flags["high_cardinality_count"] / summary.n_cols if summary.n_cols > 0 else 0)
