...
def report(
    ...
    # Используем новую опцию top_k_categories
    top_cats = top_categories(df, top_k=top_k_categories)

    # 2. Качество в целом с новыми параметрами
    ...
    # 4. Markdown-отчёт с информацией о новых настройках
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        if quality_flags['has_constant_columns']:
            f.write(f"  - Количество константных колонок: **{quality_flags['constant_columns_count']}**\n")
            if quality_flags['constant_columns']:
                f.write(f"  - Константные колонки: {', '.join(quality_flags['constant_columns'][:5])}")
                if len(quality_flags['constant_columns']) > 5:
                    f.write(f" и ещё {len(quality_flags['constant_columns']) - 5}...")
                f.write("\n")
        
        f.write(f"- Есть категориальные признаки с высокой кардинальностью (> {high_cardinality_threshold}): **{quality_flags['has_high_cardinality_categoricals']}**\n")
        if quality_flags['has_high_cardinality_categoricals']:
            f.write(f"  - Количество таких колонок: **{quality_flags['high_cardinality_count']}**\n")
            for col_info in quality_flags['high_cardinality_columns'][:3]:
                f.write(f"  - `{col_info['name']}`: {col_info['unique']} уникальных значений (доля: {col_info['cardinality_ratio']:.2%})\n")
            if len(quality_flags['high_cardinality_columns']) > 3:
                f.write(f"  - ... и ещё {len(quality_flags['high_cardinality_columns']) - 3} колонок\n")
    ...
    # 5. Картинки (используем новую опцию max_hist_columns)
    ...
    typer.echo(f"  - Порог высокой кардинальности: {high_cardinality_threshold}")
