...
class QualityFlagsResponse(BaseModel):
    ...
    flags: dict[str, bool | int | float | list | dict] = Field(
        ...,
...
async def quality_flags_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    ...
    # Используем функцию compute_quality_flags с нашими новыми эвристиками из HW03
    # По умолчанию threshold=100, как в HW03
    flags_all = compute_quality_flags(summary, missing_df, high_cardinality_threshold=100)
    ...
    return QualityFlagsResponse(
        flags=flags_all,  # Возвращаем ВСЕ флаги, включая сложные типы
        dataset_shape={"n_rows": n_rows, "n_cols": n_cols},
        latency_ms=latency_ms,
